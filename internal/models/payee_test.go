...
func TestMatchType(t *testing.T) {
	t.Run("AllMatchTypes returns all types", func(t *testing.T) {
		...
		if len(types) != expected {
			t.Errorf("Expected %d match types, got %d", expected, len(types))
		}
	})
...
func TestNewPayeeWithCategory(t *testing.T) {
	t.Run("Creates payee with category", func(t *testing.T) {
		categoryID := NewID()
		payee := NewPayeeWithCategory("Kroger", categoryID)

		if payee.ID.IsNil() {
			t.Error("NewPayeeWithCategory should create non-nil ID")
		}
		if payee.Name != "Kroger" {
			t.Errorf("Expected name 'Kroger', got %q", payee.Name)
		}
		if !payee.DefaultCategoryID.Valid {
			t.Error("NewPayeeWithCategory should set default category")
		}
		if payee.DefaultCategoryID.ID != categoryID {
			t.Errorf("Expected category ID %s, got %s", categoryID.String(), payee.DefaultCategoryID.ID.String())
		}
		if payee.Notes.Valid {
...
func TestPayeeDefaultCategory(t *testing.T) {
	...
	t.Run("SetDefaultCategory updates UpdatedAt", func(t *testing.T) {
		payee := NewPayee("Test")
		original := payee.UpdatedAt
		categoryID := NewID()

		payee.SetDefaultCategory(categoryID)

		if !payee.UpdatedAt.After(original) && !payee.UpdatedAt.Time().Equal(original.Time()) {
	...
	t.Run("ClearDefaultCategory removes category", func(t *testing.T) {
		categoryID := NewID()
		payee := NewPayeeWithCategory("Test", categoryID)
...
func TestPayeeNotes(t *testing.T) {
	t.Run("SetNotes sets notes", func(t *testing.T) {
		...
		payee.SetNotes("Some notes about this payee")

		if !payee.Notes.Valid {
			t.Error("Notes should be valid after setting")
		}
		if payee.Notes.String != "Some notes about this payee" {
			t.Errorf("Expected notes to be set, got %q", payee.Notes.String)
		}
	})
...
func TestPayeeValidation(t *testing.T) {
	...
	t.Run("Name exceeding max length fails validation", func(t *testing.T) {
		payee := validPayee()
		payee.Name = string(make([]byte, 256))
		errs := payee.Validate()
		if !errs.HasErrors() {
	...
	t.Run("Name at max length passes validation", func(t *testing.T) {
		payee := validPayee()
		payee.Name = string(make([]byte, 255))
		errs := payee.Validate()
		if errs.HasErrors() {
	...
	t.Run("Notes exceeding max length fails validation", func(t *testing.T) {
		payee := validPayee()
		payee.Notes = NullableString{String: string(make([]byte, 2001)), Valid: true}
		errs := payee.Validate()
		if !errs.HasErrors() {
	...
	t.Run("Notes at max length passes validation", func(t *testing.T) {
		payee := validPayee()
		payee.Notes = NullableString{String: string(make([]byte, 2000)), Valid: true}
		errs := payee.Validate()
		if errs.HasErrors() {
...
func TestAliasValidation(t *testing.T) {
	validAlias := func() *Alias {
		return NewExactAlias(NewID(), "TEST PATTERN")
	}
	...
	t.Run("Invalid regex pattern fails validation", func(t *testing.T) {
		alias := NewRegexAlias(NewID(), "[invalid")
		errs := alias.Validate()
		if !errs.HasErrors() {
	...
	t.Run("Valid regex pattern passes validation", func(t *testing.T) {
		alias := NewRegexAlias(NewID(), "^AMZN.*MKTP$")
		errs := alias.Validate()
		if errs.HasErrors() {
