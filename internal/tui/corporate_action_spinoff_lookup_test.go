...
// TestSpinOffDialog_LookupPrefill_AnchorsPriceCursor guards the cursor after a
// quote lookup fills the Spin-Off Price field.
func TestSpinOffDialog_LookupPrefill_AnchorsPriceCursor(t *testing.T) {
	database := dbtest.New(t)
	...
	msg, ok := cmd().(spinOffPriceLookupMsg)
	if !ok {
		t.Fatal("expected spinOffPriceLookupMsg")
	}
	app.handleSpinOffPriceLookupResult(msg)
