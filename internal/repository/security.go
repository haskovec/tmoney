...
// SecurityFilter defines optional filters for listing securities.
...
// SecurityRepository provides database operations for securities.
type SecurityRepository struct {
	db *db.DB
}

// NewSecurityRepository creates a new SecurityRepository.
func NewSecurityRepository(database *db.DB) *SecurityRepository {
	return &SecurityRepository{db: database}
}

// Create inserts a new security into the database.
func (r *SecurityRepository) Create(security *models.Security) error {
	...
	if exists {
		return &DuplicateError{Entity: "security", Field: "ticker+currency", Value: security.Ticker + "+" + security.Currency}
	}
...
func (r *SecurityRepository) GetByID(id models.ID) (*models.Security, error) {
	query := `
	...
	if err == sql.ErrNoRows {
		return nil, &NotFoundError{Entity: "security", ID: id.String()}
	}
	if err != nil {
...
func (r *SecurityRepository) GetByTicker(ticker string, currency string) (*models.Security, error) {
	var query string
	var args []any
	...
	if err == sql.ErrNoRows {
		return nil, &NotFoundError{Entity: "security", ID: ticker}
	}
	if err != nil {
...
func (r *SecurityRepository) List(filter SecurityFilter) ([]*models.Security, error) {
	query := `
		SELECT id, ticker, name, security_type, asset_class, currency,
			exchange, hidden, created_at, updated_at
		FROM securities
...
func (r *SecurityRepository) Update(security *models.Security) error {
	security.Touch()
	...
	if count == 0 {
		return &NotFoundError{Entity: "security", ID: security.ID.String()}
	}
...
func (r *SecurityRepository) Delete(id models.ID) error {
	...
	if rowsAffected == 0 {
		return &NotFoundError{Entity: "security", ID: id.String()}
	}
