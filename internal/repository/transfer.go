...
func NewTransferRepository(database *db.DB) *TransferRepository {
...
func (r *TransferRepository) Create(pair *models.TransferPair) error {
...
func (r *TransferRepository) GetByTransferID(transferID models.ID) (*models.TransferPair, error) {
	query := `
	...
	if len(transactions) == 0 {
		return nil, &NotFoundError{Entity: "transfer", ID: transferID.String()}
	}
...
func (r *TransferRepository) GetOtherSide(transactionID models.ID) (*models.Transaction, error) {
	...
	if err == sql.ErrNoRows {
		return nil, &NotFoundError{Entity: "transfer counterpart", ID: transactionID.String()}
	}
	if err != nil {
...
func (r *TransferRepository) Update(pair *models.TransferPair) error {
	// Validate the transfer pair
	if errors := pair.Validate(); errors.HasErrors() {
		return fmt.Errorf("invalid transfer pair: %v", errors)
	}
...
func (r *TransferRepository) UpdateAmount(transferID models.ID, newAmount models.Money) error {
...
func (r *TransferRepository) UpdateDate(transferID models.ID, newDate models.Date) error {
	pair, err := r.GetByTransferID(transferID)
	if err != nil {
		return err
	}
...
func (r *TransferRepository) UpdateMemo(transferID models.ID, memo string) error {
	pair, err := r.GetByTransferID(transferID)
	if err != nil {
		return err
	}
...
func (r *TransferRepository) UpdateStatus(transferID models.ID, status models.TransactionStatus) error {
	pair, err := r.GetByTransferID(transferID)
	if err != nil {
		return err
	}
...
func (r *TransferRepository) Delete(transferID models.ID) error {
	pair, err := r.GetByTransferID(transferID)
	if err != nil {
		return err
	}
...
func (r *TransferRepository) DeleteByTransactionID(transactionID models.ID) error {
...
func (r *TransferRepository) IsTransfer(transactionID models.ID) (bool, error) {
	txn, err := r.txnRepo.GetByID(transactionID)
	if err != nil {
		return false, err
	}
...
func (r *TransferRepository) ListByAccount(accountID models.ID) ([]*models.Transaction, error) {
	query := `
		SELECT id, account_id, date, amount, payee_id, category_id,
			memo, check_number, status, transfer_id, transfer_account_id,
			bank_reference_id, created_at, updated_at
...
func (r *TransferRepository) CountByAccount(accountID models.ID) (int, error) {
	var count int
