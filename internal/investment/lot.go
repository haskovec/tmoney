...
type Lot struct {
	...
	OriginalShares      types.Quantity `json:"original_shares"`
	CostPerShare        types.Money    `json:"cost_per_share"`
	PurchaseDate        types.Date     `json:"purchase_date"`
	SourceTransactionID types.ID       `json:"source_transaction_id"`
...
func NewLot(accountID, securityID types.ID, shares types.Quantity, costPerShare types.Money, purchaseDate types.Date, sourceTransactionID types.ID) Lot {
	return Lot{
...
// CostBasis returns the current cost basis: shares x cost_per_share.
func (l *Lot) CostBasis() types.Money {
...
func (l *Lot) Reduce(amount types.Quantity) error {
...
func (l *Lot) Validate() types.ValidationErrors {
