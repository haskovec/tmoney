...
type mockCategoryResolver struct {
	categories map[string]models.ID
}

func (m *mockCategoryResolver) ResolveCategoryByName(name string) (models.ID, error) {
...
type mockPayeeResolver struct {
	payees map[string]struct {
		id         models.ID
		defaultCat models.NullableID
	}
}

func (m *mockPayeeResolver) ResolvePayee(name string) (models.ID, models.NullableID, error) {
...
func (m *mockTransactionStore) ListByAccount(_ models.ID) ([]*models.Transaction, error) {
	return m.transactions, nil
}

func (m *mockTransactionStore) GetPayeeName(payeeID models.ID) string {
...
func (m *mockTransactionStore) GetBankReferenceID(txn *models.Transaction) string {
...
func (m *mockTransactionCreator) CreateTransaction(txn *models.Transaction) error {
	m.createN++
	if m.failOn > 0 && m.createN == m.failOn {
		return &models.ValidationError{Field: "test", Message: "deliberate failure"}
	}
	m.created = append(m.created, txn)
	return nil
}

func (m *mockTransactionCreator) CreateTransactionWithSplits(txn *models.Transaction, splits []*models.Split) error {
	m.createN++
	if m.failOn > 0 && m.createN == m.failOn {
		return &models.ValidationError{Field: "test", Message: "deliberate failure"}
	}
	m.created = append(m.created, txn)
	if m.splits == nil {
		m.splits = make(map[string][]*models.Split)
	}
	m.splits[txn.ID.String()] = splits
	return nil
}

func (m *mockTransactionCreator) UpdateTransaction(txn *models.Transaction) error {
...
func makeTestImportService(
	...
	existingTxns []*models.Transaction,
...
func makeMoney(s string) models.Money {
	return models.MustNewMoney(s)
}

func makeTxnWithPayee(accountID models.ID, date string, amount string, payeeID models.ID) *models.Transaction {
	txn := models.NewTransaction(accountID, makeDate(date), makeMoney(amount))
	if !payeeID.IsNil() {
...
func TestImportService_Preview_WithMatching(t *testing.T) {
	...
	svc, _ := makeTestImportService(nil, nil, []*models.Transaction{existingTxn}, payeeNames, nil)
...
func TestImportService_Preview_AutoCategorize(t *testing.T) {
	...
	categories := map[string]models.ID{"Food": categoryID}
	payees := map[string]struct {
		id         models.ID
		defaultCat models.NullableID
	}{
		"Coffee Shop": {
			id:         payeeID,
			defaultCat: models.NullableID{ID: categoryID, Valid: true},
		},
	}
...
func TestImportService_Preview_ExplicitCategoryOverridesDefault(t *testing.T) {
	accountID := models.NewID()
	payeeID := models.NewID()
	defaultCatID := models.NewID()
	explicitCatID := models.NewID()
	...
	payees := map[string]struct {
		...
		"Coffee Shop": {
			id:         payeeID,
			defaultCat: models.NullableID{ID: defaultCatID, Valid: true},
		},
	}
...
func TestImportService_Preview_FITIDMatching(t *testing.T) {
	...
	txn := models.NewTransaction(accountID, makeDate("2024-01-15"), makeMoney("-50.00"))
...
func TestImportService_Execute_UpdateMatched(t *testing.T) {
	...
	svc, creator := makeTestImportService(nil, nil, []*models.Transaction{existingTxn}, payeeNames, nil)
	...
	if creator.updated[0].Status != models.TransactionStatusCleared {
		t.Errorf("updated status = %q, want %q", creator.updated[0].Status, models.TransactionStatusCleared)
	}
}
...
func TestParseImportStatus(t *testing.T) {
	tests := []struct {
		...
		{"C", models.TransactionStatusCleared},
		{"c", models.TransactionStatusCleared},
		{"X", models.TransactionStatusCleared},
		{"x", models.TransactionStatusCleared},
		{"R", models.TransactionStatusReconciled},
		{"r", models.TransactionStatusReconciled},
		{"*", models.TransactionStatusReconciled},
		{"U", models.TransactionStatusUncleared},
		{"", models.TransactionStatusUncleared},
		{"  ", models.TransactionStatusUncleared},
	}
...
func TestImportService_Execute_StatusFromImport(t *testing.T) {
	...
	if creator.created[0].Status != models.TransactionStatusCleared {
		t.Errorf("status = %q, want %q", creator.created[0].Status, models.TransactionStatusCleared)
	}
}
