...
func TestType(t *testing.T) {
	...
	t.Run("String returns correct value", func(t *testing.T) {
		if TypeChecking.String() != "checking" {
			t.Errorf("Expected 'checking', got %q", TypeChecking.String())
		}
		if TypeCreditCard.String() != "credit_card" {
			t.Errorf("Expected 'credit_card', got %q", TypeCreditCard.String())
		}
	})
	...
	t.Run("DisplayName returns human-readable names", func(t *testing.T) {
		tests := []struct {
			...
			{TypeHSA, "HSA"},
			{TypeHSAInvestment, "HSA Investment"},
			{TypeCash, "Cash"},
			{TypeLoan, "Loan"},
			{TypeAsset, "Asset"},
		}
	...
	t.Run("IsInvestmentType returns true for investment-like types", func(t *testing.T) {
		investmentTypes := []Type{TypeInvestment, TypeHSAInvestment}
	...
	t.Run("IsInvestmentType returns false for all other types", func(t *testing.T) {
		nonInvestmentTypes := []Type{
			...
			TypeHSA, TypeCash, TypeLoan, TypeAsset,
		}
...
func TestParseType(t *testing.T) {
	t.Run("Parses valid type", func(t *testing.T) {
		at, err := ParseType("checking")
		if err != nil {
			t.Errorf("Unexpected error: %v", err)
		}
		if at != TypeChecking {
			t.Errorf("Expected TypeChecking, got %q", at)
		}
	})

	t.Run("Parses uppercase type", func(t *testing.T) {
		at, err := ParseType("SAVINGS")
		if err != nil {
			t.Errorf("Unexpected error: %v", err)
		}
		if at != TypeSavings {
			t.Errorf("Expected TypeSavings, got %q", at)
		}
	})

	t.Run("Parses mixed case type", func(t *testing.T) {
		at, err := ParseType("Credit_Card")
		if err != nil {
			t.Errorf("Unexpected error: %v", err)
		}
		if at != TypeCreditCard {
			t.Errorf("Expected TypeCreditCard, got %q", at)
		}
	})
...
func TestNewAccount(t *testing.T) {
	t.Run("Creates account with required fields", func(t *testing.T) {
		...
		if !acc.OpeningBalance.Equal(types.MustNewMoney("1000.00")) {
...
func TestAccountValidation(t *testing.T) {
	...
	t.Run("Zero opening date fails validation", func(t *testing.T) {
		acc := validAccount()
		acc.OpeningDate = types.ZeroDate
		errs := acc.Validate()
		if !errs.HasErrors() {
	...
	t.Run("Future opening date fails validation", func(t *testing.T) {
		acc := validAccount()
		acc.OpeningDate = types.Today().AddYears(1)
		errs := acc.Validate()
		if !errs.HasErrors() {
	...
	t.Run("Negative credit limit fails validation", func(t *testing.T) {
		acc := validAccount()
		acc.Type = TypeCreditCard
		acc.SetCreditLimit(types.MustNewMoney("-1000"))
		errs := acc.Validate()
		if !errs.HasErrors() {
	...
	t.Run("Zero credit limit fails validation", func(t *testing.T) {
		acc := validAccount()
		acc.Type = TypeCreditCard
		acc.SetCreditLimit(types.ZeroMoney)
		errs := acc.Validate()
		if !errs.HasErrors() {
	...
	t.Run("Positive credit limit passes validation", func(t *testing.T) {
		acc := validAccount()
		acc.Type = TypeCreditCard
		acc.SetCreditLimit(types.MustNewMoney("5000"))
		errs := acc.Validate()
		if errs.HasErrors() {
	...
	t.Run("Interest rate below 0 fails validation", func(t *testing.T) {
		acc := validAccount()
		acc.Type = TypeLoan
		acc.SetInterestRate(types.MustNewMoney("-5"))
		errs := acc.Validate()
		if !errs.HasErrors() {
	...
	t.Run("Interest rate above 100 fails validation", func(t *testing.T) {
		acc := validAccount()
		acc.Type = TypeLoan
		acc.SetInterestRate(types.MustNewMoney("150"))
		errs := acc.Validate()
		if !errs.HasErrors() {
	...
	t.Run("Valid interest rate passes validation", func(t *testing.T) {
		acc := validAccount()
		acc.Type = TypeLoan
		acc.SetInterestRate(types.MustNewMoney("5.5"))
		errs := acc.Validate()
		if errs.HasErrors() {
...
func TestAccountOptionalFields(t *testing.T) {
	...
	t.Run("SetAccountNumber accepts alphanumeric values", func(t *testing.T) {
		acc := NewAccount("Test", TypeInvestment, "USD", types.ZeroMoney, types.Today())
	...
	t.Run("SetCreditLimit sets valid value", func(t *testing.T) {
		acc := NewAccount("Test", TypeCreditCard, "USD", types.ZeroMoney, types.Today())
		acc.SetCreditLimit(types.MustNewMoney("5000"))
		if !acc.CreditLimit.Valid {
			t.Error("CreditLimit should be valid after SetCreditLimit")
		}
		if !acc.CreditLimit.Money.Equal(types.MustNewMoney("5000")) {
	...
	t.Run("SetInterestRate sets valid value", func(t *testing.T) {
		acc := NewAccount("Test", TypeLoan, "USD", types.ZeroMoney, types.Today())
		acc.SetInterestRate(types.MustNewMoney("5.5"))
		if !acc.InterestRate.Valid {
			t.Error("InterestRate should be valid after SetInterestRate")
		}
		if !acc.InterestRate.Money.Equal(types.MustNewMoney("5.5")) {
...
func TestAccountTrackLots(t *testing.T) {
	t.Run("TrackLots defaults to false", func(t *testing.T) {
		acc := NewAccount("Brokerage", TypeInvestment, "USD", types.ZeroMoney, types.Today())
		if acc.TrackLots {
	...
	t.Run("Non-investment account with TrackLots=true fails validation", func(t *testing.T) {
		...
		for _, at := range nonInvestmentTypes {
			acc := NewAccount("Test", at, "USD", types.ZeroMoney, types.Today())
			acc.TrackLots = true
			errs := acc.Validate()
			if !errs.HasErrors() {
