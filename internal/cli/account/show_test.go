...
func TestAccountShow_MissingFile(t *testing.T) {
	...
	err := cli.ExecuteWith([]string{"account", "show", "Test Account"}, stdout, stderr)
	if err == nil {
		t.Fatal("cli.ExecuteWith(account show) without --file should return error")
	}
...
func TestAccountShow_MissingNameArg(t *testing.T) {
	stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
	err := cli.ExecuteWith([]string{"account", "show", "--file", "x.tdb"}, stdout, stderr)
	if err == nil {
		t.Fatal("cli.ExecuteWith(account show) without positional name should return error")
	}
...
func TestAccountShow_ValidAccount(t *testing.T) {
	...
	stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
	if err := cli.ExecuteWith([]string{"account", "show", "Test Checking", "--file", dbPath}, stdout, stderr); err != nil {
...
func TestAccountShow_ShortFileFlag(t *testing.T) {
	...
	stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
	if err := cli.ExecuteWith([]string{"account", "show", "Quick", "-f", dbPath}, stdout, stderr); err != nil {
		t.Fatalf("cli.ExecuteWith(account show -f): %v\nstderr=%s", err, stderr)
	}
...
func TestAccountShow_RejectsExtraArgs(t *testing.T) {
	stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
	err := cli.ExecuteWith([]string{"account", "show", "A", "B", "--file", "x.tdb"}, stdout, stderr)
	if err == nil {
		t.Fatal("cli.ExecuteWith(account show ... extra) should return error")
	}
}
...
func TestAccountShow_Help(t *testing.T) {
	...
	stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
	if err := cli.ExecuteWith([]string{"account", "show", "--help"}, stdout, stderr); err != nil {
		t.Fatalf("cli.ExecuteWith(account show --help): %v", err)
	}
	if !strings.Contains(stdout.String(), "show") {
