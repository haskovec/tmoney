// Package clitest holds shared test fixtures for the per-noun CLI test
// packages. Each fixture builds an on-disk DuckDB database populated with the
// accounts, securities, and prices the CLI command tests exercise, then closes
// it and returns the file path so a test can drive the CLI against it.
//
// clitest is a regular (non-_test) package — built on the standard
// testing.T-helper pattern — so that both internal white-box tests
// (package <noun>) and external black-box tests (package <noun>_test) across
// the CLI subpackages can import it. It depends only on domain packages plus
// testing, and never on internal/cli, which would form an import cycle
// (<noun>(test) → clitest → cli → <noun>).
package clitest
