...
func TestTransferEdit_SetsCategory(t *testing.T) {
	dbPath, checking, savings := clitest.SetupTransferAccounts(t)
	billsID := seedTransferCategory(t, dbPath, "Bills")
	...
	if err := cli.ExecuteWith([]string{
		...
		t.Fatalf("transfer edit --category: %v\nstderr=%s", err, stderr)
	}
	...
	for _, leg := range []*transaction.Transaction{pair.FromTransaction, pair.ToTransaction} {
		if !leg.CategoryID.Valid || leg.CategoryID.ID != billsID {
			t.Errorf("leg %s category = %+v, want %s", leg.ID, leg.CategoryID, billsID)
		}
	}
...
func TestTransferEdit_ClearsCategory(t *testing.T) {
	dbPath, checking, savings := clitest.SetupTransferAccounts(t)
	billsID := seedTransferCategory(t, dbPath, "Bills")
	...
	func() {
		svc := clitest.OpenSvc(t, dbPath)
		pair, err := svc.Transaction.CreateTransfer(checking.ID, savings.ID, types.Today(), types.MustNewMoney("75.00"), "", types.NullableID{ID: billsID, Valid: true})
		if err != nil {
			t.Fatalf("CreateTransfer: %v", err)
		}
		legID = pair.FromTransaction.ID
	...
	if pair.FromTransaction.CategoryID.Valid || pair.ToTransaction.CategoryID.Valid {
		t.Errorf("expected both legs cleared, got from=%+v to=%+v",
			pair.FromTransaction.CategoryID, pair.ToTransaction.CategoryID)
	}
}

func TestTransferEdit_PreservesCategoryWhenUnset(t *testing.T) {
	dbPath, checking, savings := clitest.SetupTransferAccounts(t)
	billsID := seedTransferCategory(t, dbPath, "Bills")
	...
	if err := cli.ExecuteWith([]string{
		...
		t.Fatalf("transfer edit --amount: %v\nstderr=%s", err, stderr)
	}
	...
	for _, leg := range []*transaction.Transaction{pair.FromTransaction, pair.ToTransaction} {
		if !leg.CategoryID.Valid || leg.CategoryID.ID != billsID {
			t.Errorf("leg %s category = %+v, want preserved %s", leg.ID, leg.CategoryID, billsID)
		}
	}
}
...
func TestTransferEdit_PreservesCategory_ViaInvestmentLeg(t *testing.T) {
	dbPath, checking, brokerage, _, _ := clitest.SetupTransferDispatchAccounts(t)
	billsID := seedTransferCategory(t, dbPath, "Bills")

	// Seed a categorized reg→inv transfer (category lives on the Checking leg),
	// capturing the investment-side leg id — the id a user would copy from an
	// investment-account transaction listing.
	var invLegID types.ID
	func() {
		svc := clitest.OpenSvc(t, dbPath)
		res, err := svc.Investment.DepositFromAccount(brokerage.ID, checking.ID, types.Today(), types.MustNewMoney("500.00"), "", types.NullableID{ID: billsID, Valid: true})
		if err != nil {
			t.Fatalf("DepositFromAccount: %v", err)
		}
		invLegID = res.InvestmentTransaction.ID
	}()

	// Edit only the amount, addressing the transfer by its investment leg.
	stdout, stderr := &bytes.Buffer{}, &bytes.Buffer{}
	if err := cli.ExecuteWith([]string{
		"transfer", "edit", "--file", dbPath, "--txn-id", invLegID.String(),
		"--amount", "600.00",
	}, stdout, stderr); err != nil {
		t.Fatalf("transfer edit via investment leg: %v\nstderr=%s", err, stderr)
	}
	...
	if !legs[0].CategoryID.Valid || legs[0].CategoryID.ID != billsID {
		t.Errorf("category not preserved on investment-leg edit: got %+v, want %s", legs[0].CategoryID, billsID)
	}
}

func TestTransferEdit_InvToInv_CategoryRejected(t *testing.T) {
	dbPath, _, brokerage, ira, _ := clitest.SetupTransferDispatchAccounts(t)

	var invLegID types.ID
	func() {
		svc := clitest.OpenSvc(t, dbPath)
		res, err := svc.Investment.TransferCashBetweenInvestments(brokerage.ID, ira.ID, types.Today(), types.MustNewMoney("100.00"), "")
		if err != nil {
			t.Fatalf("TransferCashBetweenInvestments: %v", err)
		}
		invLegID = res.SourceTransaction.ID
	}()
	...
	if err == nil {
		t.Fatal("expected rejection of --category on inv→inv edit")
	}
	if !strings.Contains(err.Error(), "investment-to-investment") {
		t.Errorf("error = %q, want mention of investment-to-investment", err.Error())
	}
...
func TestTransferEdit_UnknownCategory_Errors(t *testing.T) {
	dbPath, checking, savings := clitest.SetupTransferAccounts(t)
