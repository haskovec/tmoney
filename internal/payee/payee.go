...
type Payee struct {
	...
	DefaultCategoryID types.NullableID     `json:"default_category_id"`
...
func NewPayee(name string) *Payee {
	return &Payee{
		...
		Name:              name,
		DefaultCategoryID: types.NullableID{Valid: false},
		Notes:             types.NullableString{Valid: false},
	}
}

// NewPayeeWithCategory creates a new Payee with a default category.
func NewPayeeWithCategory(name string, categoryID types.ID) *Payee {
	return &Payee{
		BaseModel:         types.NewBaseModel(),
		Name:              name,
...
func (p *Payee) SetDefaultCategory(categoryID types.ID) {
	p.DefaultCategoryID = types.NullableID{ID: categoryID, Valid: true}
	p.Touch()
}

// ClearDefaultCategory removes the default category.
func (p *Payee) ClearDefaultCategory() {
	p.DefaultCategoryID = types.NullableID{Valid: false}
	p.Touch()
}
...
func (p *Payee) SetNotes(notes string) {
	if notes == "" {
		p.Notes = types.NullableString{Valid: false}
	} else {
		p.Notes = types.NullableString{String: notes, Valid: true}
	}
	p.Touch()
}
...
func (p *Payee) Validate() types.ValidationErrors {
...
type Alias struct {
	...
	// Core properties
	PayeeID   types.ID  `json:"payee_id"`
	Pattern   string    `json:"pattern"`
	MatchType MatchType `json:"match_type"`
}

// NewAlias creates a new Alias with generated ID and timestamps.
func NewAlias(payeeID types.ID, pattern string, matchType MatchType) *Alias {
	return &Alias{
		BaseModel: types.NewBaseModel(),
		PayeeID:   payeeID,
		Pattern:   pattern,
...
func (a *Alias) Validate() types.ValidationErrors {
	v := types.NewValidator()
	...
	if !a.MatchType.IsValid() {
		v.AddError("match_type", "must be a valid match type (exact, contains, starts_with, or regex)")
	}
	...
	if a.MatchType == MatchTypeRegex && a.Pattern != "" {
		...
		if err != nil {
			v.AddError("pattern", fmt.Sprintf("invalid regular expression: %v", err))
		}
	}
