-- Migration 002: Transaction status expansion
-- Renames 'pending' status to 'uncleared' and adds 'void' status.
-- Updates views to exclude void transactions from balance calculations.
...
-- Step 4: Recreate transactions table with updated CHECK constraint and default
...
-- Step 5: Restore transactions data, renaming 'pending' to 'uncleared'
...
SELECT
    id, account_id, date, amount, payee_id, category_id,
    memo, check_number,
    CASE WHEN status = 'pending' THEN 'uncleared' ELSE status END,
...
-- Step 10: Recreate views with updated logic
-- account_balances: current_balance excludes void transactions
...
-- category_spending: exclude void transactions
