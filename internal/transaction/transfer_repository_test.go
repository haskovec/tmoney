...
func TestTransferRepository_Create(t *testing.T) {
	t.Run("creates valid transfer pair", func(t *testing.T) {
		...
		pair := NewTransferPair(checking.ID, savings.ID, date, amount)
		...
		if !retrieved.FromTransaction.Amount.Equal(types.MustNewMoney("-500.00")) {
		...
		if !retrieved.ToTransaction.Amount.Equal(types.MustNewMoney("500.00")) {
	...
	t.Run("rejects transfer to same account", func(t *testing.T) {
		...
		amount := types.MustNewMoney("500.00")
		date := types.NewDate(now.Year(), now.Month(), now.Day())
		pair := NewTransferPair(checking.ID, checking.ID, date, amount)
	...
	t.Run("rejects transfer with non-existent from account", func(t *testing.T) {
		...
		amount := types.MustNewMoney("500.00")
		date := types.NewDate(now.Year(), now.Month(), now.Day())
		pair := NewTransferPair(fakeAccountID, savings.ID, date, amount)
	...
	t.Run("rejects transfer with non-existent to account", func(t *testing.T) {
		...
		amount := types.MustNewMoney("500.00")
		date := types.NewDate(now.Year(), now.Month(), now.Day())
		pair := NewTransferPair(checking.ID, fakeAccountID, date, amount)
...
func TestTransferRepository_GetOtherSide(t *testing.T) {
	...
	t.Run("returns error for non-transfer transaction", func(t *testing.T) {
		...
		txn := NewTransaction(checking.ID, types.NewDate(now.Year(), now.Month(), now.Day()), types.MustNewMoney("-50.00"))
...
func TestTransferRepository_Update(t *testing.T) {
	t.Run("updates both sides of transfer", func(t *testing.T) {
		...
		pair.FromTransaction.Amount = types.MustNewMoney("-750.00")
		pair.ToTransaction.Amount = types.MustNewMoney("750.00")
		...
		if !retrieved.FromTransaction.Amount.Equal(types.MustNewMoney("-750.00")) {
			t.Errorf("Expected from amount -750.00, got %s", retrieved.FromTransaction.Amount.String())
		}
		if !retrieved.ToTransaction.Amount.Equal(types.MustNewMoney("750.00")) {
...
func TestTransferRepository_UpdateAmount(t *testing.T) {
	t.Run("updates amount on both sides", func(t *testing.T) {
		...
		// Update amount
		newAmount := types.MustNewMoney("1000.00")
		...
		if !retrieved.FromTransaction.Amount.Equal(types.MustNewMoney("-1000.00")) {
			t.Errorf("Expected from amount -1000.00, got %s", retrieved.FromTransaction.Amount.String())
		}
		if !retrieved.ToTransaction.Amount.Equal(types.MustNewMoney("1000.00")) {
...
func TestTransferRepository_UpdateDate(t *testing.T) {
	t.Run("updates date on both sides", func(t *testing.T) {
		...
		// Update date
		newDate := types.NewDate(2024, 6, 15)
...
func TestTransferRepository_UpdateStatus(t *testing.T) {
	t.Run("updates status on both sides", func(t *testing.T) {
		...
		// Update status
		if err := transferRepo.UpdateStatus(pair.FromTransaction.TransferID.ID, StatusCleared); err != nil {
		...
		if retrieved.FromTransaction.Status != StatusCleared {
			t.Errorf("Expected from status cleared, got %s", retrieved.FromTransaction.Status)
		}
		if retrieved.ToTransaction.Status != StatusCleared {
...
func TestTransferRepository_ListByAccount(t *testing.T) {
	t.Run("lists transfer transactions for account", func(t *testing.T) {
		...
		regularTxn := NewTransaction(checking.ID, date, types.MustNewMoney("-50.00"))
	...
	t.Run("returns empty list for account without transfers", func(t *testing.T) {
		...
		regularTxn := NewTransaction(checking.ID, types.NewDate(now.Year(), now.Month(), now.Day()), types.MustNewMoney("-50.00"))
		if err := txnRepo.Create(regularTxn); err != nil {
			t.Fatalf("Create regular transaction error = %v", err)
		}
...
func TestTransferRepository_TransferWorkflow(t *testing.T) {
	t.Run("full transfer lifecycle", func(t *testing.T) {
		...
		now := time.Now()
		checking := account.NewAccount("Checking", account.TypeChecking, "USD",
			types.MustNewMoney("5000.00"), types.NewDate(now.Year(), now.Month(), now.Day()))
		savings := account.NewAccount("Savings", account.TypeSavings, "USD",
			types.MustNewMoney("1000.00"), types.NewDate(now.Year(), now.Month(), now.Day()))
		...
		if err := transferRepo.UpdateAmount(pair.FromTransaction.TransferID.ID, types.MustNewMoney("750.00")); err != nil {
			t.Fatalf("UpdateAmount error = %v", err)
		}
		...
		if !updatedPair.FromTransaction.Amount.Equal(types.MustNewMoney("-750.00")) {
		...
		if clearedPair.FromTransaction.Status != StatusCleared {
			t.Error("Expected from transaction to be cleared")
		}
		if clearedPair.ToTransaction.Status != StatusCleared {
