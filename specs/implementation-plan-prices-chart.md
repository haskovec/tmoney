...
- [x] **PC-014 — Cursor-change hook in list-mode key handlers**
  ...
  - Completed 2026-05-03. The production wiring already shipped under PC-013 (front-loaded in `handlePriceListKeys` at `internal/tui/price_view.go:571-617` — every cursor-moving branch sets `cursorMoved = true`, the trailing `if cursorMoved` returns `a.schedulePriceChartFetch(a.listCursorSecurityID())`). PC-014 added the missing test pins. Two new tests in `internal/tui/price_view_test.go`: `TestHandlePriceListKeys_CursorMovingKeysScheduleDebounce` is a 7-row table-driven test (Up, Home, g, End, G, PgUp, PgDown — Down is already pinned by PC-013(a) `TestHandlePriceListKeys_DownSchedulesDebounceTick`) — each subtest positions the cursor at a known starting row, asserts the keypress returns a non-nil cmd, asserts `chartDebounceGen` strictly bumps, asserts the cursor lands on the expected row, then runs the cmd through `runDebounceTick` and asserts both `tick.secID == listCursorSecurityID()` and `tick.gen == chartDebounceGen`. `TestHandlePriceListKeys_NonCursorKeysDoNotScheduleDebounce` is a 2-row table covering `/` (search-mode entry) and `u` (refresh-prices) — asserts neither bumps `chartDebounceGen` nor moves the cursor. Total 13 new subtests across the two tables. Tests: `go fix ./...`, `go fmt ./...` clean; `go build ./...` clean; `go test ./internal/tui/ -run 'TestHandlePriceListKeys' -v` 13 pass; `go test ./...` 4773 pass / 0 fail across 23 packages (+11 net vs PC-013 baseline of 4762); `golangci-lint run` 0 issues. Coverage: `internal/tui` 68.9% → 69.1% (+0.2%); every other package held at the PC-013 baseline exactly: `cmd/tmoney` 82.7% · `internal/account` 86.1% · `internal/app` 75.7% · `internal/backup` 76.3% · `internal/category` 78.6% · `internal/config` 75.0% · `internal/db` 68.9% · `internal/dberrors` 0.0% · `internal/dbutil` 0.0% · `internal/imexport` 87.7% · `internal/investment` 81.1% · `internal/payee` 83.8% · `internal/price` 87.2% · `internal/reconciliation` 72.5% · `internal/report` 83.0% · `internal/scheduled` 76.9% · `internal/security` 89.6% · `internal/transaction` 77.1% · `internal/transferlink` 0.0% · `internal/types` 64.6% · `internal/undo` 88.0% · `tests/integration` no statements.

## Phase 7: Cache Invalidation Wiring

- [x] **PC-015 — Per-security eviction on CRUD messages**
  ...
  - Completed 2026-05-03. Two-part implementation: (1) added `App.evictSelectedSecurityFromHistoryCache()` helper in `internal/tui/price_view.go` (guards against nil `priceView` / `historyCache` / `selectedSecurity`, then calls `historyCache.Evict(selectedSecurity.ID)`); wired it in to all four CRUD handlers in `internal/tui/app.go` immediately before each `reloadPriceViewKeepingMode()` return — `priceAddedMsg`, `priceUpdatedMsg`, `priceDeletedMsg`, `priceImportedMsg`. (2) Discovered a hidden prerequisite: `loadPriceViewData` and `loadPriceViewDataForSecurity` each construct a *fresh* `historyCache` (PC-012), so the `priceViewDataLoadedMsg` handler — which fires shortly after every CRUD-triggered reload — would replace the just-Evicted cache wholesale and silently undo PC-015 (and would also undo PC-016's upcoming Clear). Fixed by having the `priceViewDataLoadedMsg` handler in `app.go` re-attach the existing `a.priceView.historyCache` to `msg.data` before swapping (`if a.priceView != nil && a.priceView.historyCache != nil { msg.data.historyCache = a.priceView.historyCache }`), preserving all surviving entries across reload while still creating one fresh cache on first load (when `a.priceView == nil`). For `priceImportedMsg` the spec says evict only `selectedSecurity.ID`; in practice CSV imports can touch arbitrary tickers, but the import dialog only opens from detail mode (`'i'` in `handlePriceDetailKeys`) so the user-visible chart is for `selectedSecurity` and that's the only entry that must be invalidated to fix the immediate render — followed the spec verbatim. Five new tests in `internal/tui/price_view_test.go` (helper `pc015TestApp` builds a populated 2-entry cache, `selectedSec` + `otherSec`): four parallel tests `TestPriceViewUpdate_PriceAddedMsg_EvictsSelectedSecurityFromCache` / `…PriceUpdatedMsg…` / `…PriceDeletedMsg…` / `…PriceImportedMsg…` each dispatch the message via `app.Update()` and assert `Lookup(selectedID)` returns `ok=false` and `Lookup(otherID)` returns `ok=true` immediately after Update returns (the reload cmd is intentionally not run — the assertion is on the synchronous side-effect of the handler). Plus `TestPriceViewDataLoadedMsg_PreservesExistingHistoryCache` pins the load-handler change: dispatches a `priceViewDataLoadedMsg` whose `msg.data.historyCache` is fresh-empty, asserts the pre-existing cache entry survives. Tests: `go fix ./...`, `go fmt ./...` clean; `go build ./...` clean; `go test ./internal/tui/ -run 'PreservesExistingHistoryCache|EvictsSelectedSecurityFromCache' -v` 5 pass; `go test ./...` 4778 pass / 0 fail across 23 packages (+5 net vs PC-014 baseline of 4773); `golangci-lint run` 0 issues. Coverage: `internal/tui` 69.1% → 69.2% (+0.1%); every other package held at the PC-014 baseline exactly: `cmd/tmoney` 82.7% · `internal/account` 86.1% · `internal/app` 75.7% · `internal/backup` 76.3% · `internal/category` 78.6% · `internal/config` 75.0% · `internal/db` 68.9% · `internal/dberrors` 0.0% · `internal/dbutil` 0.0% · `internal/imexport` 87.7% · `internal/investment` 81.1% · `internal/payee` 83.8% · `internal/price` 87.2% · `internal/reconciliation` 72.5% · `internal/report` 83.0% · `internal/scheduled` 76.9% · `internal/security` 89.6% · `internal/transaction` 77.1% · `internal/transferlink` 0.0% · `internal/types` 64.6% · `internal/undo` 88.0% · `tests/integration` no statements.

- [x] **PC-016 — Full cache clear on bulk refresh**
  ...
  - Completed 2026-05-03. The wiring landed in `internal/tui/app.go`'s `priceRefreshCompleteMsg` handler: after the status-bar notification but *before* the stale-view reload commands, when both `priceView` and `priceView.historyCache` are non-nil the handler calls `historyCache.Clear()`. The result-side cache survival (PC-015) was already in place — `priceViewDataLoadedMsg` re-attaches the existing cache onto the freshly-loaded data, so the `Clear()` here actually persists across the reload that the same handler triggers, instead of being immediately overwritten by the new `loadPriceViewData`'s fresh empty cache. (The PC-015 commit's PR-comment block in `app.go:1398-1406` had already pre-named PC-016 in anticipation of this dependency.) Spec rationale: bulk refresh can silently change *any* subset of tickers' prices, and the `RefreshResult` does not enumerate which entries actually mutated — surgical eviction would require correlating `OutcomeUpdated` entries against the in-cache ID set, which is more code for no behavioral gain over `Clear()`. One new test in `refresh_prices_test.go`: `TestRefreshCompleteMsg_ClearsHistoryCache` seeds a 2-entry cache (AAPL, MSFT) on a `priceView` in list mode, dispatches a `priceRefreshCompleteMsg` carrying a single-ticker result, and asserts both `Lookup(secA.ID)` and `Lookup(secB.ID)` return `ok=false` — the latter is the load-bearing assertion (it proves the clear is *broad*, not just an evict for the ticker named in the result). Tests: `go fix ./...`, `go fmt ./...` clean; `go build ./...` clean; `go test ./internal/tui/ -run 'TestRefreshCompleteMsg_ClearsHistoryCache' -v` passes; `go test ./...` 4779 pass / 0 fail across 23 packages (+1 net vs PC-015 baseline of 4778); `golangci-lint run` 0 issues. Coverage: held at the PC-015 baseline exactly: `cmd/tmoney` 82.7% · `internal/account` 86.1% · `internal/app` 75.7% · `internal/backup` 76.3% · `internal/category` 78.6% · `internal/config` 75.0% · `internal/db` 68.9% · `internal/dberrors` 0.0% · `internal/dbutil` 0.0% · `internal/imexport` 87.7% · `internal/investment` 81.1% · `internal/payee` 83.8% · `internal/price` 87.2% · `internal/reconciliation` 72.5% · `internal/report` 83.0% · `internal/scheduled` 76.9% · `internal/security` 89.6% · `internal/transaction` 77.1% · `internal/transferlink` 0.0% · `internal/tui` 69.2% · `internal/types` 64.6% · `internal/undo` 88.0% · `tests/integration` no statements.
